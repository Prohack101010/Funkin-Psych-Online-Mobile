...
class ReplayPlayer extends FlxBasic {
    ...
    override function update(elapsed:Float) {
        var shiftMult = (state.mobilePad?.getButtonFromName.get('buttonY')?.pressed == true || FlxG.keys.pressed.SHIFT) ? 3 : 1;
        ...
        else if (state.mobilePad?.getButtonFromName.get('buttonX')?.justPressed == true || state.controls.RESET) {
