...
class SkinsState extends MusicBeatState {
    ...
    override function update(elapsed) {
        ...
        if (mobilePad.getButtonFromName.get('buttonX').pressed || FlxG.keys.pressed.SHIFT) {
		...
		if (mobilePad.getButtonFromName.get('buttonV').justPressed || FlxG.keys.justPressed.CONTROL) {
			if (selectTimer != null)
		...
		if (mobilePad.getButtonFromName.get('buttonD').justPressed || FlxG.keys.justPressed.EIGHT) {
		...
		if (mobilePad.getButtonFromName.get('buttonC').justPressed || FlxG.keys.justPressed.TAB) {
			flipped = !flipped;
			skipStaticDestroy = true;
			LoadingState.loadAndSwitchState(new SkinsState());
		}

		if (mobilePad.getButtonFromName.get('buttonY').justPressed || FlxG.keys.justPressed.F1) {
		...
		if (mobilePad.getButtonFromName.get('buttonZ').justPressed || FlxG.keys.justPressed.F2) {
