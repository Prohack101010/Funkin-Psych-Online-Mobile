    ...
    override function update(elapsed:Float) {
		if (!searchInput.hasFocus) {
			...
			if (!LoadingScreen.loading) {
				if (FlxG.mouse.wheel == 1 || (mobilePad.getButtonFromName.get('buttonLeft').justPressed || FlxG.keys.justPressed.Q)) {
					loadNextPage(-1);
				}
				if (FlxG.mouse.wheel == -1 || (mobilePad.getButtonFromName.get('buttonRight').justPressed || FlxG.keys.justPressed.E)) {
					loadNextPage(1);
				}
