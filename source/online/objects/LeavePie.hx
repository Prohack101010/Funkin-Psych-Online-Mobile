...
class LeavePie extends FlxTypedSpriteGroup<FlxSprite> {
	...
	override function update(elapsed) {
		...
		if ((Std.isOfType(getState(), states.PlayState) && getState().mobilePad?.getButtonFromName.get('buttonP')?.pressed == true) || getState().controls.pressed('back') && !ChatBox.instance.focused) {
			exitTip.alpha = 1;
