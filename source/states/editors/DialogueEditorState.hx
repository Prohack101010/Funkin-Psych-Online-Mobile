...
class DialogueEditorState extends MusicBeatState
{
	...
	override function update(elapsed:Float) {
		...
		if(!blockInput) {
			...
			if(FlxG.keys.justPressed.SPACE || mobilePad.getButtonFromName.get('buttonY').justPressed) {
				reloadText(false);
			}
			if(FlxG.keys.justPressed.ESCAPE || mobilePad.getButtonFromName.get('buttonB').justPressed) {
				FlxG.switchState(() -> new states.editors.MasterEditorMenu());
				states.TitleState.playFreakyMusic();
				transitioning = true;
			}
			var negaMult:Array<Int> = [1, -1];
			var controlAnim:Array<Bool> = [FlxG.keys.justPressed.W || mobilePad.getButtonFromName.get('buttonUp').justPressed, FlxG.keys.justPressed.S || mobilePad.getButtonFromName.get('buttonDown').justPressed];
			var controlText:Array<Bool> = [FlxG.keys.justPressed.D || mobilePad.getButtonFromName.get('buttonRight').justPressed, FlxG.keys.justPressed.A || mobilePad.getButtonFromName.get('buttonLeft').justPressed];
			...
			if(FlxG.keys.justPressed.O || mobilePad.getButtonFromName.get('buttonA').justPressed ) {
				...
				changeText();
			} else if(FlxG.keys.justPressed.P || mobilePad.getButtonFromName.get('buttonX').justPressed) {
