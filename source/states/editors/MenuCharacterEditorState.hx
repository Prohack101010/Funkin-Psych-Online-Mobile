...
class MenuCharacterEditorState extends MusicBeatState
{
	...
	override function update(elapsed:Float) {
		...
		if(!blockInput) {
			...
			var shiftMult:Int = 1;
			if(FlxG.keys.pressed.SHIFT || mobilePad.getButtonFromName.get('buttonA').pressed) shiftMult = 10;

			if(FlxG.keys.justPressed.LEFT || mobilePad.getButtonFromName.get('buttonLeft').justPressed) {
				characterFile.position[0] += shiftMult;
				updateOffset();
			}
			if(FlxG.keys.justPressed.RIGHT || mobilePad.getButtonFromName.get('buttonRight').justPressed) {
				characterFile.position[0] -= shiftMult;
				updateOffset();
			}
			if(FlxG.keys.justPressed.UP || mobilePad.getButtonFromName.get('buttonUp').justPressed) {
				characterFile.position[1] += shiftMult;
				updateOffset();
			}
			if(FlxG.keys.justPressed.DOWN || mobilePad.getButtonFromName.get('buttonDown').justPressed) {
				characterFile.position[1] -= shiftMult;
				updateOffset();
			}

			if(FlxG.keys.justPressed.SPACE || mobilePad.getButtonFromName.get('buttonC').justPressed && curTypeSelected == 1) {
