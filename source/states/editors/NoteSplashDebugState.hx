...
class NoteSplashDebugState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.A || mobilePad.getButtonFromName.get('buttonUp').justPressed) changeSelection(-1);
		else if (FlxG.keys.justPressed.D || mobilePad.getButtonFromName.get('buttonDown').justPressed) changeSelection(1);
		...
		if(selecArr != null)
		{
			var movex = 0;
			var movey = 0;
			if(FlxG.keys.justPressed.LEFT || mobilePad.getButtonFromName.get('buttonLeft2').justPressed) movex = -1;
			else if(FlxG.keys.justPressed.RIGHT || mobilePad.getButtonFromName.get('buttonRight2').justPressed) movex = 1;

			if(FlxG.keys.justPressed.UP || mobilePad.getButtonFromName.get('buttonUp2').justPressed) movey = 1;
			else if(FlxG.keys.justPressed.DOWN || mobilePad.getButtonFromName.get('buttonDown2').justPressed) movey = -1;
			
			if(FlxG.keys.pressed.SHIFT || mobilePad.getButtonFromName.get('buttonZ').pressed)
			{
				movex *= 10;
				movey *= 10;
			}
		...
		{
			if(FlxG.keys.justPressed.C || mobilePad.getButtonFromName.get('buttonC').justPressed)
			{
			...
			else if((FlxG.keys.justPressed.V || mobilePad.getButtonFromName.get('buttonV').justPressed))
			{
			if (copiedArray != null){
		...
		if (FlxG.keys.justPressed.SPACE || mobilePad.getButtonFromName.get('buttonY').justPressed) changeAnim();
		else if (FlxG.keys.justPressed.S || mobilePad.getButtonFromName.get('buttonLeft').justPressed) changeAnim(-1);
		else if (FlxG.keys.justPressed.W || mobilePad.getButtonFromName.get('buttonRight').justPressed) changeAnim(1);

		// Force frame
		var updatedFrame:Bool = false;
		if(updatedFrame = FlxG.keys.justPressed.Q || mobilePad.getButtonFromName.get('buttonX').justPressed) forceFrame--;
		else if(updatedFrame = FlxG.keys.justPressed.E || mobilePad.getButtonFromName.get('buttonE').justPressed) forceFrame++;
