...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			if ((FlxG.keys.justPressed.ESCAPE || mobilePad.getButtonFromName.get('buttonC').justPressed) && false)
			...
			if (FlxG.keys.justPressed.ENTER || mobilePad.getButtonFromName.get('buttonA').justPressed)
			{
				autosaveSong();
			...
			if(curSelectedNote != null && curSelectedNote[1] > -1) {
				if (mobilePad.getButtonFromName.get('buttonDown2').justPressed || FlxG.keys.justPressed.E)
				{
					changeNoteSustain(Conductor.stepCrochet);
				}
				if (mobilePad.getButtonFromName.get('buttonUp2').justPressed || FlxG.keys.justPressed.Q)
				{
					changeNoteSustain(-Conductor.stepCrochet);
				}
			}
			...
			if (FlxG.keys.justPressed.BACKSPACE || mobilePad.getButtonFromName.get('buttonB').justPressed) {
			...
			if(mobilePad.getButtonFromName.get('buttonV').justPressed || FlxG.keys.justPressed.Z && FlxG.keys.pressed.CONTROL) {
				undo();
			}

			if(FlxG.keys.justPressed.Z || mobilePad.getButtonFromName.get('buttonZ').justPressed && curZoom > 0 && !FlxG.keys.pressed.CONTROL) {
				--curZoom;
				updateZoom();
			}
			if(FlxG.keys.justPressed.X || mobilePad.getButtonFromName.get('buttonD').justPressed && curZoom < zoomList.length-1) {
				curZoom++;
				updateZoom();
			}
			...
			if (FlxG.keys.justPressed.SPACE || mobilePad.getButtonFromName.get('buttonX').justPressed)
			{
			...
			if ((FlxG.keys.pressed.W || FlxG.keys.pressed.S) || (mobilePad.getButtonFromName.get('buttonUp').pressed || mobilePad.getButtonFromName.get('buttonDown').pressed))
			{
				FlxG.sound.music.pause();

				var holdingShift:Float = 1;
				if (FlxG.keys.pressed.CONTROL) holdingShift = 0.25;
				else if (FlxG.keys.pressed.SHIFT || mobilePad.getButtonFromName.get('buttonY').pressed) holdingShift = 4;

				var daTime:Float = 700 * FlxG.elapsed * holdingShift;

				if (FlxG.keys.pressed.W || mobilePad.getButtonFromName.get('buttonUp').pressed)
				{
			...
			if (FlxG.keys.pressed.SHIFT || mobilePad.getButtonFromName.get('buttonY').pressed){
				style = 3;
			}
			...
			if (FlxG.keys.justPressed.D || mobilePad.getButtonFromName.get('buttonRight').justPressed)
				changeSection(curSec + shiftThing);
			if (FlxG.keys.justPressed.A || mobilePad.getButtonFromName.get('buttonLeft').justPressed) {
				if(curSec <= 0) {
		...
		if (!holdingShift && pressedRB || holdingShift && holdingRB)
			playbackSpeed += 0.01;
		if (mobilePad.getButtonFromName.get('buttonG').justPressed || (FlxG.keys.pressed.ALT && (pressedLB || pressedRB || holdingLB || holdingRB)))
			playbackSpeed = 1;
		//
