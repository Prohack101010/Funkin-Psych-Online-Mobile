...
class FreeplayState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		if (!searchInputWait && (mobilePad.getButtonFromName.get('buttonS').justPressed || FlxG.keys.justPressed.F)) {
		...
		var shiftMult:Int = 1;
		if(mobilePad.getButtonFromName.get('buttonZ').pressed || FlxG.keys.pressed.SHIFT) shiftMult = 3;
		...
		if (!selected) {
			if(songs.length > 0)
			{
				...
				if ((mobilePad.getButtonFromName.get('buttonF').justPressed || controls.FAV) && curSelected != -1) {
				...
				if (((mobilePad.getButtonFromName.get('buttonR').justReleased && resetTotalHeld <= 3.5) || controls.RESET) && curSelected != -1 && !FlxG.keys.pressed.ALT) {
					var songId = songs[curSelected].songName + '-' + songs[curSelected].folder;
			...
			if (mobilePad.getButtonFromName.get('buttonR').pressed && resetTotalHeld <= 3.5)
			{
				resetTotalHeld += elapsed;
				if (resetTotalHeld >= 3.5)
					doSongReset = true;
			} else if (mobilePad.getButtonFromName.get('buttonR').released)
				resetTotalHeld = 0;

			if ((mobilePad.getButtonFromName.get('buttonR').pressed && doSongReset) || (controls.RESET && FlxG.keys.pressed.ALT)) {
				doSongReset = false;
			...
			if (searchGroupVList.length > 0) {
				...
				if (mobilePad.getButtonFromName.get('buttonG').justPressed || FlxG.keys.justPressed.CONTROL) {
			...
			if(mobilePad.getButtonFromName.get('buttonX').justPressed || FlxG.keys.justPressed.SPACE)
			{
				if (curSelected == -1) {
			...
			if (chatBox == null && mobilePad.getButtonFromName.get('buttonY').justPressed || FlxG.keys.justPressed.TAB) {
				persistentUpdate = false;
		...
		updateTexts(elapsed);
		if (mobilePad.getButtonFromName.get('buttonZ').pressed || FlxG.keys.pressed.SHIFT && !selected) {
